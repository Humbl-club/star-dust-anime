-- ====================================================================
-- COMPLETE ANIVAULT DATABASE SCHEMA - OPTIMIZED FOR PERFORMANCE & SECURITY
-- ====================================================================
...
-- Anime indexes
CREATE INDEX CONCURRENTLY idx_anime_title_search ON public.anime USING gin(to_tsvector('english', title));
CREATE INDEX CONCURRENTLY idx_anime_mal_id ON public.anime(mal_id) WHERE mal_id IS NOT NULL;
CREATE INDEX CONCURRENTLY idx_anime_anilist_id ON public.anime(anilist_id) WHERE anilist_id IS NOT NULL;
CREATE INDEX CONCURRENTLY idx_anime_score_desc ON public.anime(score DESC NULLS LAST);
CREATE INDEX CONCURRENTLY idx_anime_popularity_desc ON public.anime(popularity DESC NULLS LAST);
CREATE INDEX CONCURRENTLY idx_anime_year_desc ON public.anime(year DESC NULLS LAST);
CREATE INDEX CONCURRENTLY idx_anime_status ON public.anime(status);
CREATE INDEX CONCURRENTLY idx_anime_type ON public.anime(type);
CREATE INDEX CONCURRENTLY idx_anime_genres ON public.anime USING gin(genres);
CREATE INDEX CONCURRENTLY idx_anime_next_episode ON public.anime(next_episode_date) WHERE next_episode_date IS NOT NULL;
CREATE INDEX CONCURRENTLY idx_anime_updated_at ON public.anime(updated_at DESC);

-- Manga indexes
CREATE INDEX CONCURRENTLY idx_manga_title_search ON public.manga USING gin(to_tsvector('english', title));
CREATE INDEX CONCURRENTLY idx_manga_mal_id ON public.manga(mal_id) WHERE mal_id IS NOT NULL;
CREATE INDEX CONCURRENTLY idx_manga_anilist_id ON public.manga(anilist_id) WHERE anilist_id IS NOT NULL;
CREATE INDEX CONCURRENTLY idx_manga_score_desc ON public.manga(score DESC NULLS LAST);
CREATE INDEX CONCURRENTLY idx_manga_popularity_desc ON public.manga(popularity DESC NULLS LAST);
CREATE INDEX CONCURRENTLY idx_manga_status ON public.manga(status);
CREATE INDEX CONCURRENTLY idx_manga_type ON public.manga(type);
CREATE INDEX CONCURRENTLY idx_manga_genres ON public.manga USING gin(genres);
CREATE INDEX CONCURRENTLY idx_manga_updated_at ON public.manga(updated_at DESC);

-- User list indexes
CREATE INDEX CONCURRENTLY idx_user_anime_lists_user_status ON public.user_anime_lists(user_id, status);
CREATE INDEX CONCURRENTLY idx_user_anime_lists_anime ON public.user_anime_lists(anime_id);
CREATE INDEX CONCURRENTLY idx_user_manga_lists_user_status ON public.user_manga_lists(user_id, status);
CREATE INDEX CONCURRENTLY idx_user_manga_lists_manga ON public.user_manga_lists(manga_id);

-- Social indexes
CREATE INDEX CONCURRENTLY idx_reviews_anime_created ON public.reviews(anime_id, created_at DESC);
CREATE INDEX CONCURRENTLY idx_reviews_manga_created ON public.reviews(manga_id, created_at DESC);
CREATE INDEX CONCURRENTLY idx_reviews_user_created ON public.reviews(user_id, created_at DESC);
CREATE INDEX CONCURRENTLY idx_review_reactions_review ON public.review_reactions(review_id);
CREATE INDEX CONCURRENTLY idx_user_follows_follower ON public.user_follows(follower_id);
CREATE INDEX CONCURRENTLY idx_user_follows_following ON public.user_follows(following_id);
CREATE INDEX CONCURRENTLY idx_activity_feed_user_created ON public.activity_feed(user_id, created_at DESC);

-- Sync indexes
CREATE INDEX CONCURRENTLY idx_content_sync_status_type_status ON public.content_sync_status(content_type, status);
CREATE INDEX CONCURRENTLY idx_sync_logs_type_created ON public.sync_logs(content_type, created_at DESC);
...
SELECT 
  ...
  COUNT(CASE WHEN activity_type = 'rated' THEN 1 END) as ratings
...
VALUES ('database_schema_setup', 'completed', '{"message": "Complete optimized database schema deployed successfully"}');