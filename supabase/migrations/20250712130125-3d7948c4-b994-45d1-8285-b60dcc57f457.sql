...
-- Adding remaining missing foreign key indexes and removing truly unused indexes
...
-- Phase 2: Remove unused indexes that are not being utilized by queries
-- These were created but are not being used, so removing them reduces maintenance overhead
