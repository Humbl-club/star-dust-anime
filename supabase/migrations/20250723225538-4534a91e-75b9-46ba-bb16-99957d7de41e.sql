-- Fix database optimization issues - Safe version
...
-- 4. Safely unschedule existing cron jobs (ignore if they don't exist)
DO $$
BEGIN
  -- Try to unschedule, ignore errors if they don't exist
  BEGIN
    PERFORM cron.unschedule('refresh-trending-content');
  EXCEPTION WHEN OTHERS THEN
    NULL; -- Ignore errors
  END;
  
  BEGIN
    PERFORM cron.unschedule('refresh-user-stats');
  EXCEPTION WHEN OTHERS THEN
    NULL; -- Ignore errors
  END;
  
  BEGIN
    PERFORM cron.unschedule('safe-refresh-trending-content');
  EXCEPTION WHEN OTHERS THEN
    NULL; -- Ignore errors
  END;
END $$;
