-- Create missing tables for enhanced data population strategy (fixed)
...
-- Create title_people table for general staff relationships if not exists
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'title_people') THEN
