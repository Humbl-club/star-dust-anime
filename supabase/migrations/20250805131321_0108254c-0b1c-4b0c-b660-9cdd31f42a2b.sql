-- Create title_tags junction table to link titles with content tags
...
-- Enable RLS on title_tags
ALTER TABLE public.title_tags ENABLE ROW LEVEL SECURITY;

-- Create policy for public read access
CREATE POLICY "Public read title_tags" 
ON public.title_tags 
FOR SELECT 
USING (true);

-- Create indexes for better performance
CREATE INDEX IF NOT EXISTS idx_title_tags_title_id ON public.title_tags(title_id);
CREATE INDEX IF NOT EXISTS idx_title_tags_tag_id ON public.title_tags(tag_id);
CREATE INDEX IF NOT EXISTS idx_title_tags_relevance ON public.title_tags(relevance DESC);

-- Fix potential duplicate key issues with title_studios by ensuring proper constraints
-- First check if the table has issues and recreate if needed
DO $$ 
BEGIN
  -- Add ON CONFLICT DO NOTHING to prevent duplicate key errors in future inserts
  -- This doesn't fix existing duplicates but prevents new ones
  ALTER TABLE public.title_studios DROP CONSTRAINT IF EXISTS title_studios_pkey;
  ALTER TABLE public.title_studios ADD CONSTRAINT title_studios_pkey PRIMARY KEY (id);
EXCEPTION
  WHEN OTHERS THEN
    -- If there are issues, log but continue
    RAISE NOTICE 'Could not fix title_studios constraints: %', SQLERRM;
END $$;
...
BEGIN
  ...
  -- Get tags with detailed information
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
        'id', ct.id,
  ...
  FROM title_tags tt
  JOIN content_tags ct ON tt.tag_id = ct.id
  ...
  WITH filtered_titles AS (
    ...
    WHERE 
      ...
      (tag_slugs IS NULL OR EXISTS (
        ...
        AND (ct.slug = ANY(tag_slugs) OR ct.name = ANY(tag_slugs))
      ))
      AND
...
-- Create indexes on content_tags for better performance
...
CREATE INDEX IF NOT EXISTS idx_content_tags_rank ON public.content_tags(rank DESC);
