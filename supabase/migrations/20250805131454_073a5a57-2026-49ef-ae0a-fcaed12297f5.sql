-- Create title_tags junction table to link titles with content tags (if not exists)
...
-- Enable RLS on title_tags (safe to run multiple times)
ALTER TABLE public.title_tags ENABLE ROW LEVEL SECURITY;

-- Create policy for public read access (drop and recreate to avoid conflicts)
DROP POLICY IF EXISTS "Public read title_tags" ON public.title_tags;
