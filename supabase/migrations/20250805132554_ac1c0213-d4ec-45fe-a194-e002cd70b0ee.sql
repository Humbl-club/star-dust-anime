-- Drop existing function if it exists to allow recreation with new return type
DROP FUNCTION IF EXISTS public.get_title_metadata(uuid);
...
BEGIN
  ...
  -- Get tags - return empty for now since we need title_tags junction table
  tags_json := '[]'::jsonb;
...
-- Create metadata search function (fixing parameter name conflict)
